...
#pragma omp parallel for private(iicube,i,j,k,dx,dy,dz,rho,U,V,W,VV,P,C,e0,beta,C_plan,V_in_1,Sigma_in)
			...
			for (i = n_buffer; i <= nx; i++) {
				for (j = n_buffer; j <= ny; j++) {
					for (k = n_buffer; k <= nz; k++) {  
						...
						VV = U*U+V*V+W*W;
						P = (U1_[iicube][i][j][k][4]-0.5*rho*VV)*(K-1);
						C = K*P/rho;
						...
						Sigma_in = Sigma_in_0*V_in_1/(dx*NcubeX);


						Fabs[iicube][i][j][k][0] = -(V_in_1*( U1_[iicube][i][j][k][0]-U1_[iicube][i-1][j][k][0] )/dx+Sigma_in*(rho-rho0));
						Fabs[iicube][i][j][k][1] = -(V_in_1*( U1_[iicube][i][j][k][1]-U1_[iicube][i-1][j][k][1] )/dx+Sigma_in*(rho*U-rho0*U0));
						Fabs[iicube][i][j][k][2] = -(V_in_1*( U1_[iicube][i][j][k][2]-U1_[iicube][i-1][j][k][2] )/dx+Sigma_in*(rho*V-rho0*V0));
						Fabs[iicube][i][j][k][3] = -(V_in_1*( U1_[iicube][i][j][k][3]-U1_[iicube][i-1][j][k][3] )/dx+Sigma_in*(rho*W-rho0*W0));
						Fabs[iicube][i][j][k][4] = -(V_in_1*( U1_[iicube][i][j][k][4]-U1_[iicube][i-1][j][k][4] )/dx+Sigma_in*(U1_[iicube][i][j][k][4]-e0));
...
#pragma omp parallel for private(iicube,i,j,k,dx,dy,dz,rho,U,V,W,VV,P,C,e0,beta,C_plan,V_out_1,Sigma_out)
			...
			for (i = n_buffer; i <= nx; i++) {
				for (j = n_buffer; j <= ny; j++) {
					for (k = n_buffer; k <= nz; k++) {  
						...
						Sigma_out = Sigma_out_0*V_out_1/(dx*NcubeX);


						Fabs[iicube][i][j][k][0] = -(V_out_1*( U1_[iicube][i][j][k][0]-U1_[iicube][i-1][j][k][0] )/dx+Sigma_out*(rho-rho0));
						Fabs[iicube][i][j][k][0] = -(V_out_1*( U1_[iicube][i][j][k][1]-U1_[iicube][i-1][j][k][1] )/dx+Sigma_out*(rho*U-rho0*U0));
						Fabs[iicube][i][j][k][0] = -(V_out_1*( U1_[iicube][i][j][k][2]-U1_[iicube][i-1][j][k][2] )/dx+Sigma_out*(rho*V-rho0*V0));
						Fabs[iicube][i][j][k][0] = -(V_out_1*( U1_[iicube][i][j][k][3]-U1_[iicube][i-1][j][k][3] )/dx+Sigma_out*(rho*W-rho0*W0));
						Fabs[iicube][i][j][k][0] = -(V_out_1*( U1_[iicube][i][j][k][4]-U1_[iicube][i-1][j][k][4] )/dx+Sigma_out*(U1_[iicube][i][j][k][4]-e0));
						
						
						//if(j == 10 && k == 10)
						//	printf("%f\t%f\n",Sigma_out*(rho*U-rho0*U0),Sigma_out*(rho-rho0));
